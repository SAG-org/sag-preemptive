...
namespace Preemptive {
	...
	public:
		...
		// derive a new set by "cloning" an existing set and adding an index set
		Index_set(const Index_set &from, const std::vector<std::size_t> idxs)
				: the_vector(from.the_vector.size()) {
			if (!idxs.empty()) {
				the_vector.resize(std::max(from.the_vector.size(), *std::max_element(idxs.begin(), idxs.end()) + 1));
			}
			std::copy(from.the_vector.begin(), from.the_vector.end(), the_vector.begin());
			for (auto idx: idxs) {
				the_vector[idx] = true;
		...
		friend std::ostream &operator<<(std::ostream &stream, const Index_set &s) {
			bool first = true;
			stream << "{";
